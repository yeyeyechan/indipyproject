...
class TR_1206_2(QMainWindow):
    ...
    def ReceiveData(self, rqid):
        ...
        if before_personal_vol >0 and before_foreign_vol <0 and before_program_vol <0 and before_trans_ammount > 9000000000:
            if after_personal_vol <0 and after_personal_vol >0 and after_program_vol >0 and after_trans_ammount > 9000000000:
                ...
                if self.input.find_one({'단축코드': data['단축코드']}):
                    data_input = self.input.find_one({'단축코드': data['단축코드']}).copy()
...
if __name__ == "__main__":
    ...
    for i in collection_data:
        ...
        if checkindex == len(collection_data):
            TR_1206Event.exit(0)
        checkindex += 1
        TR_1206_vari = TR_1206_2(i['단축코드'], "20200317", "20200318", '1', '0', i['종목명'], db_name, standard_length)
        time.sleep(0.3)
