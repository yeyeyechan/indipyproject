...
class SC(QMainWindow):
    ...
    def ReceiveData(self, rqid):
        ...
        # 데이터 받기
        DATA[stock_data[0]] = int(self.IndiTR.dynamicCall("GetSingleData(int)", 8))  # 전체순매수대금
        DATA[stock_data[1]] = self.IndiTR.dynamicCall("GetSingleData(int)", 1) # 단축코드

        print("SK")
        print(DATA)
        #print(int(DATA[stock_data[2]])>0 and  int(DATA[stock_data[1]])<0)
        print("SK")
        if  DATA[stock_data[0]]>10000000000:
            print("yes")
            client = MongoClient('127.0.0.1', 27017)
            db = client["stock_data"]
            collection = db["SC_result"]
            collection.insert(DATA)
                # print(collection.insert(DATA))
                # time.sleep(0.5)
               # headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
                #response =  requests.post("http://1.232.245.14:2020/mst_SK/", data= json.dumps(DATA), headers= headers)
...
if __name__ == "__main__":
    client = MongoClient('127.0.0.1', 27017)
    db = client["stock_data"]
    collection = db["test_for_2day"]
    app = QApplication(sys.argv)
    for i in collection.find():
        SK_vari = SC(i["stock_code"])
        time.sleep(0.3)
    app.exec_()