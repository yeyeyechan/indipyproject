            ...
            for times in self.timeline:
                if (int)(times)<data_time:
                    ...
                    if self.collection3.find_one({'stock_code': DATA['단축코드']}):
                        ...
                        if self.collection5.find_one({'stock_code': DATA['단축코드']}):
                            if self.collection5.find_one({'stock_code': DATA['단축코드']})['after_foreign_ratio']<current_foreign_ratio :
                                self.realTimeLogger.info("디버그용")
                                self.realTimeLogger.info(
                                    "sself.collection5.find_one({'stock_code': DATA['단축코드']})['after_foreign_ratio']<current_foreign_ratio")
                                self.realTimeLogger.info(self.collection5.find_one({'stock_code': DATA['단축코드']})[
                                                             'after_foreign_ratio'] < current_foreign_ratio)
                                self.realTimeLogger.info("디버그용")
